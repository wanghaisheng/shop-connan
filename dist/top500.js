...
app.get("/scrapetop500", async (req, res) => {
    await (0, app_1.top500)();
});
app.get("/top500", async (req, res) => {
    let uniqdomains = await (0, app_1.upsertFile)("shopify-top-500-afership.txt");
    ...
    for (let i = 0; i < uniqdomains.length; i++) {
        ...
        const done = await (0, app_1.upsertFile)('sitemaps/' + filename + '-sitemap-urls.txt');
        let sitemapurl = '';
        if (done.length > 1) {
            ...
            if (sitemapurl.length == 0) {
                ...
                url_list = await (0, app_1.parseSitemap)(sitemapurl[0]);
            }
